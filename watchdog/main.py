...
def run_election():
    ...
    for ip in get_watchdogs_ips():
        try:
            response = requests.get('http://' + ip + ':' + os.environ['HEALTHCHECK_PORT'] + '/id')
            id = response.json().get('id')
            print('Process ' + ip + ' says its id is ' + id + ' and its ' + str(response.json().get('leader')) + ' that it is the leader')
